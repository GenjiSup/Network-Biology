...
results_module <- results %>%
  filter(gene_name %in% genes.green)
library(clusterProfiler)
library(org.Hs.eg.db)

# Prepare gene list and map symbols to ENTREZ IDs
gene_symbols <- results_module$gene_name
gene_entrez <- bitr(gene_symbols, fromType = "SYMBOL", toType = "ENTREZID", OrgDb = org.Hs.eg.db)
gene_entrez_ids <- unique(gene_entrez$ENTREZID)

# Perform KEGG pathway enrichment analysis
kegg_results <- enrichKEGG(gene = gene_entrez_ids, organism = "hsa", pvalueCutoff = 0.05)

# Visualize top 10 KEGG pathways
dotplot(kegg_results, showCategory = 1)
```
